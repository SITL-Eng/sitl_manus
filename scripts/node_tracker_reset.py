...
################### INDEX TRACKING #######################
    ...
    def callback_index_relq(self,q_rel):
        self.rel_base_index = Quaternion(w=q_rel.data[3], x=q_rel.data[0], y=q_rel.data[1], z=q_rel.data[2]) 
...
###########################################################
    ...
    def resetting (self):        
        ...
        return memorized_x,memorized_y,memorized_z,memorized_rtx,memorized_rty,memorized_rtz,memorized_rtw,memorized_x_c,memorized_y_c,memorized_z_c,memorized_x_rc,memorized_y_rc,memorized_z_rc,memorized_w_rc,base_offset_x,base_offset_y,base_offset_z,last_rotation_x,last_rotation_y,last_rotation_z,last_rotation_w,offset,initialdata
    ...
    def listener(self):
        ...
        rospy.Subscriber("glove/left/on_off", BoolStamped, app.callback_on_off)
...
if __name__ == '__main__':  
    ...
    while not rospy.is_shutdown():
        if app.flag_on_off == True and previous_on_off!=app.flag_on_off:
            print("HEREEEEEE")
            memorized_x,memorized_y,memorized_z,memorized_rtx,memorized_rty,memorized_rtz,memorized_rtw,memorized_x_c,memorized_y_c,memorized_z_c,memorized_x_rc,memorized_y_rc,memorized_z_rc,memorized_w_rc,base_offset_x,base_offset_y,base_offset_z,last_rotation_x,last_rotation_y,last_rotation_z,last_rotation_w,offset,initialdata=app.resetting()

            previous_on_off = app.flag_on_off
